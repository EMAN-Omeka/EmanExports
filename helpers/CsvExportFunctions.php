...
function getOrderedElements() {
    ...
    $select = $table->getSelect()->where('element_set_id IN (1, 3)')->order('element_set_id ASC')->order('order ASC');
 ...
 * @return string[]
 */
function getCollectionCsvRow($collection, $elements) {
    $row = array();
    // Element texts
    $elementTexts = get_db()->getTable('ElementText')->findByRecord($collection);
    foreach ($elements as $element) {
...
*/
    // Public?
    $row[] = $collection->public ? '1' : '0';
    // Featured?
    $row[] = $collection->featured ? '1' : '0';
    // Done
    return $row;
}
 ...
 * @param Item[] $items
 */
function printCollectionsCsvExport($collections) {
    // Get all elements as columns
    $elements = getOrderedElements();
    ...
    foreach ($collections as $collection) {
        fputcsv($f, getCollectionCsvRow($collection, $elements), ',', '"', "\0");
    }
}